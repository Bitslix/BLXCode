...
- **BLXCode Agent settings (3-column grid)**: Settings → **BLXCode Agent** (renamed from Agent Provider; sidebar `HsCatProvider` → “BLXCode Agent”) is implemented in `src/workbench/agent_provider_pane/` with a responsive 3-column layout. **Text** (column 1): provider + thinking pickers (shared `harness-provider-trigger` dropdown chrome), API-key status line pointing at API Keys, rich model picker (menu rows show description or OpenRouter pricing; detail sub-row under selection; custom model-id field), refresh + source hint. **Image** (column 2): former Settings → Image pane extracted as `AgentImageColumn` in `harness_image_pane` (provider chips + `ModelPicker`, auto-save on change). **Voice** (column 3): placeholder copy until voice settings migrate here (full Voice tab remains for now). Web Tools sit below the grid as a 3-option radio grid (App-style); one **Save** / **Discard** footer persists text provider settings and web backend together. New i18n: `AgColumnText`, `AgColumnImage`, `AgColumnVoice`, `AgColumnVoiceSoon`, `AgSectionInference`, `AgSectionModel`, `AgModelMetaPricing`. Dedicated CSS: `agent_provider_pane.css`.
...
- **Workspace settings layout aligned with API Keys**: the Workspace pane moved into `src/workbench/workspace_settings_pane/` and now mirrors the App / API Keys harness pattern — `harness-subpane` sections (**Paths & sandbox**, **Embedded browser**), `api-keys-list` / `api-keys-row` field cards with monospace path inputs, label→input spacing on `.api-keys-row__head`, and one footer **Save** / **Discard** (native `disabled` + `.workbench-mini-btn:disabled` styling) that persists default project directory, agent sandbox root, and embedded-browser URL together (replacing three inline Save/Apply buttons). New i18n keys `WsSectionPaths` and `WsSectionBrowser` in all locales. Shared footer/field CSS lives in `workspace_settings_pane.css` and `api_keys_pane.css` (`.settings-pane-footer`, `.settings-field-card`).
...
- **Settings → Image sidebar entry**: image model configuration now lives in the middle column of **Settings → BLXCode Agent**; the dedicated Image category button was removed from the settings nav (legacy `HarnessSettingsCategory::Image` still routes to BLXCode Agent for old snapshots).
