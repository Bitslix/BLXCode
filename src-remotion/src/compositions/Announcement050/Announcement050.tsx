...
const features: FeatureSceneData[] = [
  {
    ...
    title: "Customize the agent around your workflow",
    body: "More tools, stronger security controls, richer session behavior, and new ways to shape how the BLXCode Agent works for you.",
    accent: "cyan",
    tags: ["custom roles", "tool access", "security", "workspace-aware"],
    visual: "agent",
  },
  {
    id: "mcp",
  ...
  {
    id: "local-providers",
    kicker: "[UPDATE] Local Providers",
    title: "Ollama and LM Studio get first-class treatment",
    body: "Local providers use server URLs, require no API key, and plug into the same model picker and agent loop.",
    accent: "green",
    tags: ["Ollama", "LM Studio", "no API key", "Server URL"],
    visual: "localProviders",
  },
  {
    id: "heartbeat",
...
const Orb: React.FC<{ accent: string; size?: number; pulse?: number }> = ({
  accent,
  size = 176,
  pulse = 1,
}) => {
  const frame = useCurrentFrame();
  const wobble = Math.sin(frame / 14) * 5 * pulse;
  const glow = 0.55 + Math.sin(frame / 18) * 0.14 * pulse;
  return (
    <div
      style={{
        width: size,
        height: size,
        borderRadius: "50%",
        background: `radial-gradient(circle at 35% 30%, #ffffff 0%, ${accent} 20%, #252842 58%, #0b0c12 100%)`,
        boxShadow: `0 0 ${60 + glow * 30}px ${accent}66, inset -18px -28px 52px rgba(0,0,0,0.46)`,
        transform: `rotate(${wobble}deg) translateY(${Math.sin(frame / 20) * 7}px)`,
        position: "relative",
      }}
    >
      <div
...
const AgentVisual: React.FC<{ accent: string }> = ({ accent }) => {
  const frame = useCurrentFrame();
  const toolProgress = enterValue(frame, 54, 34);
  const securityProgress = enterValue(frame, 84, 26);
  const tools = ["workspace.read", "plans.write", "terminal.keys", "settings.update", "mcp.search"];
  return (
    <VisualWrapper title="BLXCode Agent" accent={accent}>
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 20, height: "100%" }}>
        <div className="a050-card" style={{ padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
          <Orb accent={accent} size={190} />
          <Chip accent={accent}>Architect mode</Chip>
          <div style={{ width: "100%" }}>
            <div className="a050-label" style={{ marginBottom: 10 }}>security</div>
            <Progress progress={62 + securityProgress * 30} accent={accent} />
          </div>
          <div className="a050-code" style={{ color: "var(--a-muted)", textAlign: "center" }}>
            access: supervised<br />
            tools: {numberText(18 + toolProgress * 16)}
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateRows: "1fr 168px", gap: 18 }}>
          <div className="a050-card" style={{ padding: 22 }}>
            <div className="a050-label" style={{ marginBottom: 16 }}>custom tool surface</div>
            {tools.map((tool, i) => {
              const p = enterValue(frame, 26 + i * 9, 22);
              return (
                <div
                  key={tool}
                  className="a050-card"
                  style={{
                    ...
                    borderColor: i % 2 === 0 ? `${accent}55` : undefined,
                  }}
                >
                  <span className="a050-code" style={{ color: i % 2 === 0 ? accent : "var(--a-muted)" }}>{tool}</span>
                </div>
              );
            })}
          </div>
          <div className="a050-card" style={{ padding: 18 }}>
            <div className="a050-mini-title">New function hooks</div>
            <Lines rows={3} accent={accent} delay={72} />
          </div>
        </div>
      </div>
    </VisualWrapper>
...
const KanbanVisual: React.FC<{ accent: string }> = ({ accent }) => {
  ...
  const columns = [
    { name: "Backlog", cards: ["Agent roles", "Canvas view"] },
...
const ProvidersVisual: React.FC<{ accent: string; localOnly?: boolean }> = ({ accent, localOnly }) => {
  const frame = useCurrentFrame();
  const local = ["Ollama", "LM Studio"];
  const cloud = ["OpenAI", "Anthropic", "OpenRouter", "Hugging Face", "Cloudflare", "Together", "Portkey"];
  const list = localOnly ? local : [...local, ...cloud];
  return (
    <VisualWrapper title={localOnly ? "Local Providers" : "Agent Providers"} accent={accent}>
      <div style={{ display: "grid", gridTemplateColumns: localOnly ? "1fr 1fr" : "repeat(3, 1fr)", gap: 16 }}>
        {list.map((name, i) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ marginTop: localOnly ? 28 : 16 }}>
              ...
              {localOnly && (
                <div style={{ marginTop: 20 }}>
                  <Chip accent={accent}>no API key needed</Chip>
                </div>
              )}
            </div>
          );
  ...
  return (
    <VisualWrapper title="ADE-aware Agent Roles" accent={accent}>
      <div style={{ position: "relative", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Orb accent={accent} size={210} pulse={1.3} />
        {roles.map((role, i) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                borderColor: i === 0 || i === 5 ? accent : undefined,
                background: i === 0 || i === 5 ? `${accent}13` : undefined,
              }}
            >
              <div className="a050-mini-title">{name}</div>
            </div>
...
const FeatureVisual: React.FC<{ feature: FeatureSceneData; accent: string }> = ({ feature, accent }) => {
  switch (feature.visual) {
    ...
    case "localProviders":
      return <ProvidersVisual accent={accent} localOnly />;
    case "heartbeat":
  ...
  return (
    <AbsoluteFill className="a050" style={{ opacity }}>
      ...
      <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 120px" }}>
        ...
        >
          The agentic workspace just got bigger: MCP, Kanban, voice, memory, local models, remote SSH, HeartBeat, Canvas, Swarm, and a sharper app foundation.
        </p>
        <div
          style={{
            marginTop: 40,
      ...
      <Shell accent={accent} label="release / v0.5.0">
        <div style={{ height: "calc(100% - 58px)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 44, padding: 70, alignItems: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, opacity: grid }}>
            {["MCP", "Kanban", "Memory", "BLXVoice", "Remote SSH", "HeartBeat", "Canvas", "Swarm"].map((item, i) => (
