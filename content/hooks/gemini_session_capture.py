#!/usr/bin/env python3
"""blxcode: Gemini CLI SessionStart hook (cross-platform).

Same idea as ``claude_session_capture.py``: persist
{terminal_key -> {agent: gemini, session_id, ...}} so blxcode can later
issue ``gemini --resume <id>`` for the precise terminal slot.
...
Gemini's SessionStart payload contains ``session_id`` (string),
``transcript_path``, ``cwd``, ``hook_event_name``, and ``source`` —
identical naming to Claude/Codex.
"""
...
def main() -> int:
    ...
    if not terminal_key or not sessions_path:
        _diag_log(
            diag_path,
            f"gemini SKIP missing_env terminal_key={bool(terminal_key)} sessions_path={bool(sessions_path)}",
        )
        return 0
    ...
    if not isinstance(session_id, str) or not session_id.strip():
        _diag_log(
            sessions_path,
            f"gemini SKIP no_session_id key={terminal_key} payload_keys={list(payload.keys())}",
        )
        return 0
    ...
    _diag_log(
        sessions_path,
        f"gemini WRITE key={terminal_key} session_id={session_id.strip()} source={source}",
    )
