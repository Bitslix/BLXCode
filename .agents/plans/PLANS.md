...
|--------|------|-------------|
| done | [architecture-map-multi-lang.md](architecture-map-multi-lang.md) | Architecture-Map Plugin-Registry: Rust/Node/Python/CMake/Generic Indexer; Rebuild ohne Cargo.toml-Pflicht (Fix für TS/Python/C++/etc-Workspaces) |
| done | [code-wiki-architecture-map.md](code-wiki-architecture-map.md) | Architektur-Karte im Workspace-Memory: reservierte Kategorie `architecture/` + `ARCHITECTURE.md`, statischer Indexer (`memory_rebuild_architecture`), Write-Guards, Frontmatter-Roundtrip, Shared-Preload, Checklist/Skill/Scout, UI-Pin/Block/Rebuild-Button; MVP + alle Phase-2-Punkte |
...
| done | [linux-browser-iframe-boot-fix.md](linux-browser-iframe-boot-fix.md) | Linux Boot-Crash Fix: sticky Lazy Mount für BrowserTabDock + iframe-src-Gating (WebKitGTK nested iframe) |
