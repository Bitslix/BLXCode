...
| planned | [session-context-window-compaction.md](session-context-window-compaction.md) | Context-Window-Budget sichtbar machen: max Größe aus Provider-Metadaten (OpenRouter context_length + statische Fallback-Tabelle), Live-Auslastungs-Meter im Chat-Header, Compact-Button (Konversation zusammenfassen + neu starten), Auto-Compact bei ~85 %; Phasen 1–4 |
| done | [remote-ssh-file-git-resume.md](remote-ssh-file-git-resume.md) | Remote (SSH) File-/Git-Browsing + Session-Resume: persistenter Exec-Channel pro Connection, fs/git-Routing local-vs-remote, Git-Polling statt notify, Resume via remote Agent-Sessions; Phasen 0–3 |
