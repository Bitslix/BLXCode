#!/usr/bin/env bash
# Interactive BLXCode setup for macOS development and local desktop builds.
set -Eeuo pipefail
...
Options:
  ...
  --skip-system  Skip Xcode/Homebrew system setup.
...
check_platform() {
  if [[ "$(uname -s)" != "Darwin" ]]; then
    die "This script is for macOS. Use scripts/setup-linux.sh on Linux."
  fi
}
...
ensure_xcode_tools() {
  ...
  if [[ "$SKIP_SYSTEM" == "1" ]]; then
    warn "Skipping Xcode/Homebrew setup (--skip-system)."
    return
  fi

  if xcode-select -p >/dev/null 2>&1; then
    ok "Xcode Command Line Tools are available: $(xcode-select -p)"
    return
  fi

  warn "Xcode Command Line Tools are required for Tauri macOS builds."
  if [[ "$CHECK_ONLY" == "1" ]]; then
    info "Would offer: xcode-select --install"
    return
  fi
  if confirm "Open the Xcode Command Line Tools installer?"; then
    run xcode-select --install
    warn "Finish the Apple installer, then re-run this script."
  else
...
ensure_node() {
  ...
  warn "Node.js and npm are required for frontend-js. Node 22 is recommended because CI uses it."
  if [[ "$SKIP_SYSTEM" == "1" || "$CHECK_ONLY" == "1" ]]; then
    info "Install Node.js LTS from https://nodejs.org/ or use Homebrew: brew install node"
    return
  fi
  ...
  if command_exists brew; then
    if confirm "Install Node.js with Homebrew?"; then
      ...
      warn "Node install skipped. Install Node.js LTS and re-run."
    fi
  else
    warn "Homebrew was not found. Install Node.js LTS from https://nodejs.org/ and re-run."
  fi
}
...
run_verification() {
  ...
  if [[ "$WITH_BUNDLE" == "1" ]]; then
    ...
    (cd "$ROOT" && run cargo tauri build --target universal-apple-darwin)
  fi
}
...
main() {
  check_platform
  section "BLXCode macOS setup"
  info "This script sets up Rust, Tauri, Trunk, Node/npm, and macOS build tools."
